...
from ctime import get_datetime_range
...
def demonstrate_basic_datetime_range():
    ...
    for dt in get_datetime_range(start_date, end_date):
        print(f"  - {dt}")
...
def demonstrate_hourly_interval():
    ...
    for dt in get_datetime_range(start_date, end_date, step):
...
def demonstrate_with_timezone():
    ...
    try:
        ...
        print(
            f"4-hour intervals with timezone from {start_date} to {end_date}:"
        )
        for dt in get_datetime_range(start_date, end_date, step):
